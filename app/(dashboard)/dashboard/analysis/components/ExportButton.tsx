...
export default function ExportButton({
  ...
  const getVariantClasses = () => {
    switch (variant) {
      case 'primary':
        return 'text-white bg-blue-600 border border-transparent hover:bg-blue-700';
      case 'secondary':
  ...
  if (!showDropdown) {
    return (
      <button
        ...
        className={`inline-flex items-center justify-center font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${getVariantClasses()} ${getSizeClasses()} ${className}`}
      >
        {exporting === 'pdf' ? (
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <button
        onClick={handleQuickExport}
        disabled={exporting === 'pdf'}
        className={`inline-flex items-center justify-center font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${getVariantClasses()} ${getSizeClasses()}`}
      >
        {exporting === 'pdf' ? (
          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
        ) : (
      ...
      <div className="relative">
        <button
          ...
          className={`ml-2 inline-flex items-center justify-center font-medium rounded-lg transition-colors ${getVariantClasses()} ${getSizeClasses()}`}
        >
